                            ...
                            <div class="single-product-large-image">
                                <div class="tab-content">
                                    @forelse($product->getImages('xl','images') as $key=> $image)
                                        <div role="tabpanel" class="tab-pane {{$key==0 ? 'active fade in' : 'fade' }}"
                                             id="single-img-{{$key}}">

                                            <div class="product-img"
                                                 style="background-image: url({!!url(@$image)!!});"></div>
                                ...
                                <ul class="sm-img-nav" role="tablist">
                                    @forelse($product->getImages('sm','images') as $key=> $image)
                                        <li role="presentation" class="{{$key==0 ? 'active': ''}}"><a
                                                    href="#single-img-{{$key}}" aria-controls="single-img-{{$key}}"
                            ...
                            <div class="detail-block">
                                <div class="product-detail-heading">
                                    <div class="detail-heading-left">
                                        ...
                                        ?>
                                        <span class="rating"><span class="label-success label"> {!!@$average!!} <i
                                                        class="fa fa-star"></i></span> {!!Review::total($product->id)!!}
                                            Ratings & {!!Review::count($product->id)!!} Reviews </span>
                                            ...
                                            <div class="col-sm-6 text-right">
                                                @if(user('client.web'))
                                                    @if(!user('client.web')->product->contains($product->id))
                                                        <div class="favorite-button" id="fav-btn">
                                                            <a id="{!!$product->getRouteKey()!!}"
                                                               class="fav-prop btn btn-primary btn-xs"
                                                ...
                                                @else
                                                    <div class="favorite-button">
                                                        <a id="{!!$product->getRouteKey()!!}"
                                                           class="fav-prop btn btn-primary btn-xs" data-toggle="tooltip"
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-2">
                                                <input type="number" name="" id="qty" class="form-control" value="1"
                                                       min="1" max="10" step="" required="required" title="">
                                            </div>
                                            <div class="col-sm-7">
                                                <a id="{!!$product->id!!}"
                                                   class="add-cart links-item btn btn-primary"><i
                                    ...
                                    @forelse(Review::review($product->id) as $key => $review)
                                            ...
                                            <div class="media-left">
                                                <a href="#">
                                                    <img class="media-object"
                                                         src="{!!url(@$review['user']->defaultImage('md','photo'))!!}"
                                                         alt="user">
                                                </a>
                                            </div>
                                        ...
                                        @for($i=0; $i<5; $i++)
                                                ...
                                                <div class="media-footer">
                                                    <p><span>{!!@$review['user']['name']!!}
                                                            ,</span> {!!format_date(@$review->created_at)!!}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @empty
